...
void GUI::reloadCarList(vector<Car> cars) {
    this->tableCars->clearContents();
    this->tableCars->setRowCount(cars.size());

    int lineNumber=0;
    for(auto& car:cars){
        this->tableCars->setItem(lineNumber,0,new QTableWidgetItem(QString::fromStdString(car.get_nr_inmatriculare())));
        this->tableCars->setItem(lineNumber,1,new QTableWidgetItem(QString::fromStdString(car.get_producator())));
        this->tableCars->setItem(lineNumber,2,new QTableWidgetItem(QString::fromStdString(car.get_model())));
        this->tableCars->setItem(lineNumber,3,new QTableWidgetItem(QString::fromStdString(car.get_tip())));
        lineNumber++;
    }
}
...
void GUI::connectSignalSlots() {
    // adaugare
    QObject::connect(btnAddCar, &QPushButton::clicked, this,&GUI::guiAddCar);
    ...
    // Stergere
    QObject::connect(btnStergeMasina, &QPushButton::clicked, this,&GUI::guiDeleteCar);

    // Modificare
    QObject::connect(btnModifica,&QPushButton::clicked,this,&GUI::guiModifyCar);
